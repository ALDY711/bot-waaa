...
async function fetchCatalog(sock, { jid, limit = 10, cursor } = {}) {
  ...
  // Percobaan 1: Menggunakan fungsi bawaan getCatalog dengan timeout guard 8 detik
  try {
    if (typeof sock.getCatalog === 'function') {
      return await queryWithTimeout(sock.getCatalog({ jid, limit, cursor }), 8000);
    }
  } catch (err1) {
    // Percobaan 2: Direct query w:biz:catalog dengan format biz_jid
    try {
      ...
      const res2 = await queryWithTimeout(sock.query({
        tag: 'iq',
        attrs: {
          to: '@s.whatsapp.net',
          type: 'get',
      ...
      return parseCatalogNode(res2);
    } catch (err2) {
      // Percobaan 3: Fallback ekstrak produk dari koleksi (karena koleksi juga menyimpan data produk)
      try {
        const collectionsRes = await fetchCollections(sock, jid, limit);
        const collectedProducts = [];
        if (collectionsRes?.collections) {
          for (const col of collectionsRes.collections) {
            if (col.products && Array.isArray(col.products)) {
              for (const p of col.products) {
                if (!collectedProducts.some(existing => existing.id === p.id)) {
        ...
        if (collectedProducts.length > 0) {
          return { products: collectedProducts };
        }
      } catch (err3) {
        // Fallback koleksi gagal
      }

      throw new Error('Server WhatsApp tidak merespons query katalog (Timed Out). Server WhatsApp kemungkinan mengabaikan query w:biz:catalog untuk companion session saat ini.');
    }
  }
}
